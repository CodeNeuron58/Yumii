...
The backend serves a local API on `http://127.0.0.1:8000`. It's what the orb
and dashboard use — and it's yours to script against. It binds to localhost
only, and browsers are held to an origin allowlist (extendable via the
`YUMII_ALLOWED_ORIGINS` environment variable).
...
| `error` | `{error}` — something failed; she stays alive |
