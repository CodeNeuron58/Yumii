...
description: "How Yumii manages system prompts and custom text personas under the hood."
layout: "cards"
icon: "message-square-text"
---

<ContentCard>
Yumii leverages the flexibility of LLM system prompts to enforce distinct personalities. All system prompts are stored in structured text files inside the package assets:

*   **Prompt Directory**: `src/yumii/assets/prompts/`
*   **Default Personas**:
...
When loading an agent, Yumii's engine does not just send the raw text prompt file. To satisfy the WebSocket and Live2D sync requirements, `src/yumii/agent/llm.py` appends a **strict tool constraint** (`_TOOL_HINT`):
