...
2.  Runs `uv tool install git+https://github.com/CodeNeuron58/Yumi.git` — installs Yumi as a uv-managed tool (no manual clone, no separate venv). The CPU-only PyTorch wheel pin in `pyproject.toml` is honored automatically.
3.  The `yumi` command becomes globally available — `uv tool install` puts it on `~/.local/bin` for you, no shell alias needed.
