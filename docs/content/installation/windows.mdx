...
<StepsList>
  <div>Checks if Python 3.12+ is installed (prompts if missing).</div>
  <div>Installs the `uv` package manager securely.</div>
  <div>Clones the Yumi repository to `C:\Users\YourName\Yumi`.</div>
  <div>Creates a virtual environment and runs `uv sync` to set up all packages (pinning CPU-only Torch).</div>
  <div>Creates a global `yumi` alias so you can launch the app from any directory.</div>
</StepsList>
...
<StepsList>
  <div>Right-click the speaker icon in your Windows Taskbar and select **Sound settings**.</div>
  <div>Under **Input**, ensure your primary microphone is selected as the default device.</div>
  <div>Test your microphone to ensure the volume bar moves when you speak.</div>
</StepsList>
