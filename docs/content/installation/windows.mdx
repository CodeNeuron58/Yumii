...
<StepsList>
  <Step icon="code" title="Checks if Python 3.12+ is installed (prompts if missing)." />
  <Step icon="package" title="Installs the `uv` package manager securely." />
  <Step icon="folder" title="Clones the Yumi repository to `C:\Users\YourName\Yumi`." />
  <Step icon="terminal" title="Creates a virtual environment and runs `uv sync` to set up all packages (pinning CPU-only Torch)." />
  <Step icon="star" title="Creates a global `yumi` alias so you can launch the app from any directory." />
</StepsList>
</ContentCard>
