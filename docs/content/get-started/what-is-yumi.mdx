...
4.  **The Body**: The frontend plays the streaming audio, computes the real-time RMS wave amplitude, maps it to lip movements on a **user-supplied Live2D model** (drop a Cubism 4 `.model3.json` into `~/.yumi/avatar/` and point `modelUrl` at it in `src/yumi/assets/webui/index.html`), and applies the LLM's requested body gestures and expressions.
