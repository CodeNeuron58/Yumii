# Yumi — Project Context for AI Coding Tools
...
- **Frontend:** Vanilla HTML + PixiJS 6 + Live2D Cubism SDK (single file: `src/yumi/assets/webui/index.html`)
...
3. **Non-sensitive config** (personality, provider choice) lives in `~/.yumi/config.json`.
4. **Avatar files** (user-provided Live2D models) live in `~/.yumi/avatar/`.
5. **Asset files** (prompts, webui) live in `src/yumi/assets/` — inside the package, so they're bundled in the wheel.

---

## Entry Points
- `yumi` CLI command → `src/yumi/cli.py:app`
- `yumi wake-up` → starts FastAPI on `localhost:8000`, opens browser
...
uv run yumi          # Launch dashboard / run app
